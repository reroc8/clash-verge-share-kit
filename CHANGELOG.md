...
- 探测分两步：先看文件在不在，再确认它真的能应答。只看文件存在不够——换过运行模式之后，上一个模式留下的死 socket 文件会被选中，脚本会一直报「采样失败」却看不出原因。上游服务自己也是按「能不能连上」而不是「文件在不在」判断的。
...
- 验证方式：把三个候选路径换成不存在的位置（应列出全部候选并继续重试），以及把「文件在但没人应答」的死 socket 放在候选首位（应跳过它、命中后面的真实地址；这一条在只判断文件存在时会被选中）。
